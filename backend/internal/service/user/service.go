...
//
// Code is the 6-digit email verification code received through the public
// email-verification start flow. It is required when SMTP is configured
// (production); ignored in dev where operators commonly run without SMTP.
...
func (s *Service) AdminList(ctx context.Context, limit, offset int) ([]AdminUserView, error) {
	rows, err := s.users.List(ctx, limit, offset)
	if err != nil {
		return nil, err
	}
	...
	identitiesByUser, err := s.users.ListOIDCIdentitiesByUserIDs(ctx, userIDs)
	if err != nil {
		return nil, err
	}
	...
	for _, row := range rows {
		out = append(out, AdminUserView{
			User:       row,
			OIDCLinked: len(identitiesByUser[row.ID]) > 0,
		})
	}
	return out, nil
}
