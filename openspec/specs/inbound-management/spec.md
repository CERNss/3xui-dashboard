...
Per-node inbound CRUD plus the fleet-wide aggregated view that powers
the admin Inbounds page. Tag↔remote-id cache lives at the runtime
layer; this module owns the service-level operations.

## Purpose & boundaries

Adjacent modules: **`runtime-3xui-client`** owns the HTTP wire +
tag→id cache; **`client-provisioning`** depends on this module for
the inbound a client is created against.
