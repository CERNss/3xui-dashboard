...
The React + Ant Design platform contract that replaces the Vue 3 +
Tailwind frontend. Pins the library taxonomy, server/client state
boundary, i18n preservation rule, and the build-output shape that
the Go backend's `go:embed` relies on.

This spec defines the **platform**, not the product surface. Page
behavior continues to be specified by `admin-views`,
`layouts-and-chrome`, `unified-login`, `subscription`, and the rest
of the existing product specs — all of which remain
platform-agnostic about the choice of UI framework.
