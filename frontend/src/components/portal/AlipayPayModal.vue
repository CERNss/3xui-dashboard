...
const canDismissSoftly = computed(() => state.value !== 'waiting')
...
function closeFromSoftDismiss() {
  if (!canDismissSoftly.value) return
  close()
}

function onKey(e: KeyboardEvent) {
  if (e.key === 'Escape') closeFromSoftDismiss()
}
...
<template>
  <Teleport to="body">
    <div
      ...
      @click.self="closeFromSoftDismiss"
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="alipay-modal-title"
        class="w-full max-w-sm animate-scale-in rounded-2xl bg-surface-0 shadow-elevated dark:bg-surface-900"
      >
        <div class="px-6 pt-6">
          <div class="flex items-start justify-between">
            <div>
              <h2 id="alipay-modal-title" class="text-base font-semibold tracking-tight text-ink-900 dark:text-surface-50">{{ $t('alipay.title') }}</h2>
              <p v-if="order" class="mt-1 text-xs text-surface-500">{{ $t('alipay.orderLine', { id: order.id, amount: '¥' + (order.price_cents / 100).toFixed(2) }) }}</p>
            </div>
        ...
        <div v-if="state === 'waiting'" class="px-6 py-5">
          <div class="flex justify-center">
            <div class="rounded-2xl border border-surface-100 bg-surface-50 p-3 dark:border-surface-800 dark:bg-surface-800">
              ...
              <div v-else class="flex h-60 w-60 items-center justify-center text-xs text-surface-400">{{ $t('alipay.qrGenerating') }}</div>
            </div>
          </div>
          <p class="mt-4 text-center text-xs text-surface-500">{{ $t('alipay.scanHint') }}</p>
          <p v-if="remainingSec > 0" class="mt-1 text-center text-2xs text-surface-400 tabular-nums">
            {{ $t('alipay.qrTimeout', { time: `${Math.floor(remainingSec / 60)}:${String(remainingSec % 60).padStart(2, '0')}` }) }}
          </p>
          <a
            v-if="order?.payment_target_url"
            :href="order.payment_target_url"
            class="mt-4 inline-flex h-9 w-full items-center justify-center gap-1.5 rounded-xl bg-primary-600 px-4 text-sm font-medium text-white shadow-card transition-all hover:bg-primary-500 active:scale-[0.98]"
          >
            {{ $t('alipay.apkOpen') }}
        ...
        <div v-else-if="state === 'success'" class="px-6 py-8 text-center">
          ...
          <h3 class="mt-3 text-sm font-semibold text-ink-900 dark:text-surface-50">{{ $t('alipay.success') }}</h3>
          <p class="mt-1 text-xs text-surface-500">{{ $t('alipay.redirecting') }}</p>
        </div>
        ...
        <div v-else class="px-6 py-8 text-center">
          ...
          <h3 class="mt-3 text-sm font-semibold text-ink-900 dark:text-surface-50">
            {{ state === 'failed' ? $t('alipay.failed') : $t('alipay.expired') }}
          </h3>
          <p class="mt-1 text-xs text-surface-500">
            {{ state === 'failed' ? $t('alipay.failedHint') : $t('alipay.expiredHint') }}
          </p>
          <button
            type="button"
