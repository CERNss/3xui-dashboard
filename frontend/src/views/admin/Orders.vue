...
async function reload() {
  ...
  try {
    ...
    error.value = formatError(e, t('admin.orders.loadFailed'))
  } finally {
    loading.value = false
  }
}
...
function statusPill(s: string): { cls: string; label: string } {
  switch (s) {
    case 'completed':
    case 'paid':
      return { cls: 'bg-accent-50 text-accent-700 ring-accent-100 dark:bg-accent-950/40 dark:text-accent-300 dark:ring-accent-800', label: t('admin.orders.status.completed') }
    case 'failed':
      return { cls: 'bg-red-50 text-red-600 ring-red-100 dark:bg-red-950/40 dark:text-red-300 dark:ring-red-800', label: t('admin.orders.status.failed') }
    case 'refunded':
      return { cls: 'bg-amber-50 text-amber-700 ring-amber-100 dark:bg-amber-950/40 dark:text-amber-300 dark:ring-amber-800', label: t('admin.orders.status.refunded') }
    case 'created':
      return { cls: 'bg-surface-100 text-surface-600 ring-surface-200 dark:bg-surface-800 dark:text-surface-300 dark:ring-surface-700', label: t('admin.orders.status.created') }
    default:
...
async function refund(o: AdminOrder) {
  const ok = await askConfirm({
    ...
    message: t('admin.orders.refundConfirmMsg', {
      id: o.id,
      plan: planName(o.plan_id),
      amount: formatYuan(o.price_cents),
      email: userEmail(o.user_id),
    }),
    variant: 'danger',
    confirmLabel: t('admin.orders.refund'),
  })
  if (!ok) return
  ...
  try {
    ...
    refundErr.value = formatError(e, t('admin.orders.refundFailed'))
  } finally {
...
<template>
  <div>
    <header class="mb-7 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight text-ink-900 dark:text-surface-50">{{ $t('admin.orders.title') }}</h1>
        <p class="mt-1.5 text-sm text-surface-500 dark:text-surface-400">{{ $t('admin.orders.subtitle') }}</p>
      </div>
      <button class="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-surface-200 bg-surface-0 text-surface-600 transition-all ease-brand hover:border-surface-300 hover:bg-surface-50 hover:text-ink-900 active:scale-[0.98] dark:border-surface-700 dark:bg-surface-900 dark:hover:bg-surface-800" :title="$t('admin.orders.reload')" @click="reload">
    ...
    <div v-if="!loading && orders.length > 0" class="mb-4 flex h-9 items-center gap-0.5 rounded-xl border border-surface-200 bg-surface-0 p-1 text-xs dark:border-surface-700 dark:bg-surface-900 w-fit">
      <button v-for="s in ['all','completed','failed','refunded','created'] as const" :key="s" type="button"
        ...
        {{ s === 'all' ? $t('admin.orders.filterAll') : statusPill(s).label }}
      </button>
    </div>
      ...
      <table class="min-w-full text-sm">
        <thead class="text-left text-xs font-semibold uppercase tracking-wider text-surface-500 dark:text-surface-400">
          <tr class="border-b border-surface-100 dark:border-surface-800">
            <th class="px-6 py-3 font-medium">{{ $t('admin.orders.column.orderId') }}</th>
            <th class="px-6 py-3 font-medium">{{ $t('admin.orders.column.user') }}</th>
            <th class="px-6 py-3 font-medium">{{ $t('admin.orders.column.plan') }}</th>
            <th class="px-6 py-3 text-right font-medium">{{ $t('admin.orders.column.amount') }}</th>
            <th class="px-6 py-3 font-medium">{{ $t('admin.orders.column.status') }}</th>
            <th class="px-6 py-3 font-medium">{{ $t('admin.orders.column.created') }}</th>
            <th class="px-6 py-3 font-medium">{{ $t('admin.orders.column.completed') }}</th>
          ...
          <tr v-for="o in filtered" :key="o.id" class="transition-colors hover:bg-surface-50/60 dark:hover:bg-surface-800/40">
            ...
            <td class="px-6 py-3.5 text-right">
              <button
                ...
                @click="refund(o)"
              >
                {{ refundingId === o.id ? $t('admin.orders.processing') : $t('admin.orders.refund') }}
              </button>
            </td>
          </tr>
        </tbody>
